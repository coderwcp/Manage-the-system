...
export default {
  ...
  methods: {
    ...
    // 保存用户信息 工号 部门等信息
    async saveUser() {
      const { fileList, uploading } = this.$refs.staffPhoto
      // 当uploading为true时则表示文件正在上传
      if (!fileList.length) return this.$message.warning('请选择上传图片')
      if (uploading) return this.$message.error('正在上传图片')
      // 将头像链接更新到当前formData对象里面staffPhoto，再上传到服务器
      await saveUserDetailById({ ...this.userInfo, id: this.userId, staffPhoto: fileList.length > 0 ? fileList[0].url : undefined })
      this.$message.success('保存成功')
    },

    // 保存用户个人信息
    async savePersonal() {
      const { fileList, uploading } = this.$refs.myStaffPhoto
      if (!fileList.length) return this.$message.warning('请选择上传图片')
      // 当uploading为true时则表示文件正在上传
      if (uploading) return this.$message.error('正在上传图片')
      // 将头像链接更新到当前formData对象里面staffPhoto，再上传到服务器
      await updatePersonal({ ...this.formData, staffPhoto: fileList.length > 0 ? fileList[0].url : undefined })
      this.$message.success('保存成功')
    }
  }
}
